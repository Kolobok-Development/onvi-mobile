...
export class GazpromUsecase {
  ...
  async reference(user: Client, reference: string): Promise<any> {
    ...
    if (clientPartner) {
      try {
        return await this.gazpromRepository.getSession(
    ...
    const partnerClient = PartnerClient.create({
      metaData: JSON.stringify(clientGazprom),
      partnerUserId: partnerUserId,
    });
    await this.partnerRepository.apply(partnerClient, partner, user.clientId);
    const referenceData = await this.gazpromRepository.reference(
      reference,
      partnerUserId,
      user.correctPhone,
    );
    const subscriptionData = await this.gazpromRepository.getSubscriptionData(
      partnerClient.partnerUserId,
    );
  ...
  async activationSession(user: Client): Promise<any> {
    ...
    await this.partnerRepository.apply(partnerClient, partner, user.clientId);
    return await this.gazpromRepository.registration(
      partnerUserId,
      user.correctPhone,
    );
  }
  ...
  async updatePartnerData(
    ...
    return await this.gazpromRepository.updateData(
      clientPartner.partnerUserId,
      metaData,
    );
  }
  ...
  async updateClientData(
    input: GazpromUpdateDto,
    partner: Partner,
  ): Promise<any> {
    const { client, promotion, ...metaData } = input;
    const clientPartner =
      await this.partnerRepository.findPartnerClientByPartnerUserIdAndPartnerId(
  ...
  async cancelClientData(
    input: GazpromUpdateDto,
    partner: Partner,
  ): Promise<any> {
    const clientPartner =
      await this.partnerRepository.findPartnerClientByPartnerUserIdAndPartnerId(
        input.client.partner_user_id,
        partner.id,
      );
    if (!clientPartner) {
      throw new NotFoundException(432, 'Client not found');
    }
    const metaData = clientPartner.metaData
      ? JSON.parse(JSON.stringify(clientPartner.metaData))
      : {};
