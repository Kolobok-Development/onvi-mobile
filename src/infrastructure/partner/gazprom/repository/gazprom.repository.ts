...
export class GazpromRepository implements IGazpromRepository {
    ...
    constructor(
        ...
        this.baseUrl = 'GAZPROM_BASE_URL';
        this.apiKey = 'GAZPROM_API_KEY';
        this.partnerId = 'GAZPROM_PARTNER_ID';
    }

    async registration(clientId: number, phoneNumber: string): Promise<GazpromSessionDto | GazpromErrorDto> {
    ...
    async getSubscriptionData(clientId: number): Promise<GazpromSubscriptionResponseDto | GazpromErrorDto> {
        const config = this.setHeaders();
        ...
        try {
            ...
            return new GazpromSubscriptionResponseDto(request.data.items, request.data.count);
        } catch (err) {
            const { response } = err;
            return new GazpromErrorDto(
                response.data.code,
    ...
    async getSession(clientId: number): Promise<GazpromSessionDto | GazpromErrorDto> {
        const config = this.setHeaders();
    ...
    async updateData(clientId: number, meta: GazpromUpdateDto): Promise<GazpromUpdateResponseDto | GazpromErrorDto> {
        const config = this.setHeaders();
