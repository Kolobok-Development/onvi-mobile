...
@Injectable()
export class PartnerStrategy extends PassportStrategy(
    Strategy,
    'partner'
) {
    ...
    async validate(partnerKey: string, done: (error: Error, data) => Record<string, unknown>,) {
        try {
            const partner =
                await this.partnerRepository.findOneByToken(partnerKey);
            if (!partner) {
